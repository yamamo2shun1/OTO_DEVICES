...
| 120/121 | Disable or enable XFader curve edit mode. |
...
- `PC127` saves the complete device configuration listed in Section 5 to EEPROM, including the DVS enable states, Return and headphone source assignments, magnetic output mode, auxiliary fade-down assignments, and XFader curves.
...
- XFader curve edit mode is enabled.
...
- The default values are `0` for XFader A and `96` for XFader B.

## 4. `curve_edit_mode` (XFader Curve Edit Mode) Behavior

- Use Program Change messages to enable or disable the mode (`PC121` ON / `PC120` OFF).
- While enabled, the main OLED shows the current CC values for XFader curves A and B together with a graphical preview of each curve.
