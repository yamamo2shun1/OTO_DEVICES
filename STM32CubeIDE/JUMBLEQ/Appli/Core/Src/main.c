...
int main(void)
{
    ...
    /* MPU設定はBootで行われているため、Appliでは再設定しない */
    /* Boot側のMPU_Config()で設定済み:
     * - Region 2: 0x24000000 (256KB) Cacheable
     * - Region 3: 0x24040000 (256KB) Non-cacheable
     */
    ...
    /* Disable D-Cache for USB DMA debugging ---------------------------------*/
    /* DMAモードでのDCache coherency問題をデバッグするため一時的に無効化 */
    // SCB_EnableDCache();
    SCB_DisableDCache();
