...
// I2C送信完了待ち用セマフォとミューテックス
static SemaphoreHandle_t ssd1306_i2c_sem = NULL;    // 送信完了通知用
static SemaphoreHandle_t ssd1306_i2c_mutex = NULL;  // 排他制御用
static uint8_t ssd1306_cmd_byte = 0;  // コマンド用バッファ（非同期送信中に保持）

// I2C送信完了コールバック（HALから呼ばれる）
void HAL_I2C_MemTxCpltCallback(I2C_HandleTypeDef *hi2c) {
    if (hi2c->Instance == SSD1306_I2C_PORT.Instance) {
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        if (ssd1306_i2c_sem != NULL) {
            xSemaphoreGiveFromISR(ssd1306_i2c_sem, &xHigherPriorityTaskWoken);
...
void ssd1306_I2C_Init(void) {
    if (ssd1306_i2c_sem == NULL) {
        ssd1306_i2c_sem = xSemaphoreCreateBinary();
    }
...
static HAL_StatusTypeDef ssd1306_I2C_Write(uint16_t mem_addr, uint8_t* data, uint16_t size) {
    ...
    if (ssd1306_i2c_mutex == NULL || ssd1306_i2c_sem == NULL) {
        return HAL_ERROR;
    }
    ...
    if (xSemaphoreTake(ssd1306_i2c_mutex, pdMS_TO_TICKS(200)) == pdTRUE) {
        ...
        while (HAL_I2C_GetState(&SSD1306_I2C_PORT) != HAL_I2C_STATE_READY && timeout > 0) {
            timeout--;
        }
        
        if (timeout > 0) {
            // 割り込みモードで送信開始
            status = HAL_I2C_Mem_Write_IT(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, mem_addr, 1, data, size);
            
            if (status == HAL_OK) {
                // 送信完了を待機（最大100ms）
                if (xSemaphoreTake(ssd1306_i2c_sem, pdMS_TO_TICKS(100)) != pdTRUE) {
                    status = HAL_TIMEOUT;
                }
            }
        } else {
...
void ssd1306_WriteCommand(uint8_t byte) {
    ssd1306_cmd_byte = byte;  // staticバッファにコピー
    ssd1306_I2C_Write(0x00, &ssd1306_cmd_byte, 1);
}

// Send data
void ssd1306_WriteData(uint8_t* buffer, size_t buff_size) {
    ssd1306_I2C_Write(0x40, buffer, (uint16_t)buff_size);
}
